...
const Nav: FC<NavProps> = () => {
  ...
  const navLinks = [
    ...
    { label: t('nav.edit_account'), id: 'edit_account', route: '/edit-account' },
    ...
    { label: t('nav.dashboard'), id: 'dashboard', route: '/dashboard' },
  ];
    ...
    <nav className="bg-transparent border-0 border-b-2 border-yellow-400">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Navigation links */}
          <div className={`flex-1 flex justify-between items-center ${isMobileMenuOpen ? 'block bg-gray-700 z-10 mt-auto pt-4 w-full' : 'hidden'} md:flex`}>
            <div className="flex flex-col md:flex-row items-center space-x-2 md:space-x-2 relative">
              {navLinks.map((link) => (
                link.route === '#' ? (
                  <a
                    key={link.id}
                  ...
                  >
                    ...
                    {
                      link.id === 'select_game' && <img src="/images/caret.svg" className='inline-block p-2' />
                    }
                  </a>
                ) : (
                  <Link
